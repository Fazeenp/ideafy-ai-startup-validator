      ...
      <div className="relative z-10 w-full max-w-md bg-black/50 backdrop-blur-md border border-white/10 shadow-xl rounded-2xl p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <input type="password"
            ...
            onChange={handleChange} required
            className="w-full rounded-lg bg-white/5 border border-white/10 px-4 py-3 text-white placeholder-white/40 focus:outline-none focus:ring-2 focus:ring-pink-500" />
          <motion.button whileHover={{ scale: 1.05 }} type="submit" className="w-full py-2 cursor-pointer rounded-lg font-semibold text-lg bg-gradient-to-r from-yellow-400 via-pink-500 to-purple-500 shadow-lg" >
            Login
          </motion.button>
        </form>
        <p className="text-sm text-white/60 text-center mt-4">
          Don’t have an account?{" "}
          <span className="text-pink-400 hover:underline cursor-pointer" onClick={() => navigate("/signup")} > Sign up </span>
        </p>
      </div>
    </section>
  );
